  ...
  async start() {
    ...
    app.get("/:pageid", async (req, res) => {
      const shortURL = req.params.pageid.replace(/[^a-zA-Z0-9]/g, "");
          ...
          if(!response || response == "no-data") res.redirect("/");
      const data = {
    ...
    app.post("/send_data", async (req, res) => {
      ...
      if(whereError.length > 0){
        ...
        return this.stopRequest(res, ({"code": 400, "info": `${whereError.map(r => r).join(", and ")} are not filled`}))
      }else{

        // Yes, we can find another errors
        if(datasReceived.title.length >= 20 || datasReceived.title.length < 2) return this.stopRequest(res, {"code": 400, "info": `The page title is too long/ court (between 2 and 20 characters)`});
        if(datasReceived.description.length >= 5000 || datasReceived.description.length <= 50) return this.stopRequest(res, {"code": 400, "info": `The page description is too long/ court (between 50 and 5000 characters)`});
        
        if(datasReceived.shortUrl){
          if(datasReceived.shortUrl.length >= 10 || datasReceived.shortUrl.length <= 2) return this.stopRequest(res, {"code": 400, "info": `The short url for the page is too long/ court (between 2 and 10 characters)`});
        }

        const dateSepareted = datasReceived.date.split("-");
        if(dateSepareted.length != 3) return this.stopRequest(res, {"code": 400, "info": `The date entered is invalid`});
        const d = new Date(datasReceived.date);
        if(d == "Invalid Date") return this.stopRequest(res, {"code": 400, "info": `The date entered is invalid`});
        const now = new Date();

        if(d.getTime() < now.getTime() + 7200000) return this.stopRequest(res ,{"code": 400, "info": `The date entered is invalid`});
        ...
        if(body.shortUrl){
          ...
          if(!response || response == "no-data"){
            ...
            await this.database.piwa("INSERT INTO db (pageid, pageurl, shorturl, date, title, description) VALUES (?, ?, ?, ?, ?, ?)", [code, code, shortURL, d.getTime().replace(".0", ""), body.title, body.description], "run");
            return this.stopRequest(res ,{"code": 200, "info": "The page was created succefully", "accessCode": code})
          ...
          const code = await this.generateCodeURL(10);
          await this.database.piwa("INSERT INTO db (pageid, pageurl, date, title, description) VALUES (?, ?, ?, ?, ?)", [code, code, d.getTime().repalce(".0", ""), body.title, body.description], "run");
          return this.stopRequest(res, {"code": 200, "info": "The page was created succefully", "accessCode": code})
        }
      }
    });

    let startedWebsite = await app.listen(this.config.website.port);
  }
